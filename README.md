...
有 query：返回 === 直接命中记忆 === 和可选的 === 联想浮现 ===；直接命中默认只展示最相关的 1 条，含 [bucket_id:...]。短文本、importance>=7、anchor/pinned/protected、haven_favorite/flavor_*、valence 极值会返回原文 moment + 年轮；普通长桶仍走脱水摘要。扩散仍可用前几条命中作为种子。关键词和 embedding 双通道合并，默认过滤 feel。
...
```text
1. breath 的直接命中会按 `direct_hit_raw_policy` 保留重要原文和年轮；普通长桶仍可能是脱水摘要。要精确读完整正文/年轮请用 read_bucket(bucket_id)。
...
direct_hit_raw_policy:
  enabled: true
  short_raw_tokens: 100       # 100 token 内直接返回原文
  importance_min: 7           # importance>=7 返回原文
  valence_extreme_delta: 0.35 # abs(valence-0.5)>=0.35 返回原文
```

`anchor / pinned / protected / haven_favorite / flavor_*` 也会返回原文 moment + 年轮。这个策略只影响直接命中；`=== 联想浮现 ===` 仍然是短摘要。
